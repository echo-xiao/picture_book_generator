...
def test_feedback_rate_limited_after_quota(monkeypatch):
    ...
    ip = "203.0.113.10"  # unique IP so other tests don't share the bucket
...
def test_rate_limit_is_per_ip(monkeypatch):
    ...
    for _ in range(7):
        _post_feedback(client, "203.0.113.20")
    # A different IP is unaffected.
    assert _post_feedback(client, "203.0.113.21").status_code != 429
